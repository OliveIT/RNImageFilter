...
- (void)didSetProps:(NSArray<NSString *> *)changedProps
{
  ...
  if ([changedProps containsObject:@"isTarget"]) {
  }
}
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    if (innerSelf && innerSelf.target && innerSelf.inputImage) {
      ...
      dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [innerSelf fetchImage:image];
      });
    }
  });
}

- (void)fetchImage:(UIImage *)image {
    if (!_isTarget || _targetPath == NULL)  return;
    ...
    NSError *error = nil;
    if ([manager fileExistsAtPath:_targetPath]) {
        [manager removeItemAtPath:_targetPath error:&error];
    }
    
    [manager createFileAtPath:_targetPath contents:imageData attributes:nil];
}
